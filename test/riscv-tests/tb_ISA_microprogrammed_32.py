...
selected_prefixes = prefixes #['rv32mi-p-breakpoint']
...
def computeAllTests():
    ...
    for f in files:
        if (isElf(ex_dir + f)):
            ...
            if ('-v-' in f):
                print('Avoiding VM test', f, end=' ')
                ret[f] = ('FAILED', 'avoided')
            else:
...
def runAllTests():
    ...
    for prefix in selected_prefixes:
        ...
        if (nTotalGroup == 0):
            print(f'Group: {prefix} Total: {nTotalGroup}')    
            continue
    ...
    for prefix in selected_prefixes:
        ...
        if (nTotalGroup == 0):
            print(f'{prefix:15}', asciiProgressBar(0, 1))
            continue
