...
class FPU:
    ...
    def class_hp(self, xv):
        ...
        if (v.infinity):
            if (s == 1):
                return FP_CLASS_NEG_INF
            else:
                return FP_CLASS_POS_INF
        if (v.nan):
            if (m == 1):
                return FP_CLASS_SNAN
            else:
                return FP_CLASS_QNAN
        if (v.m == 0):
            if (s == 1):
                return FP_CLASS_NEG_ZERO
            else:
                return FP_CLASS_POS_ZERO
        if (e == 0):
            if (s == 1):
                return FP_CLASS_NEG_SUBNORMAL
            else:
                return FP_CLASS_POS_SUBNORMAL

        if (s == 1):
            return FP_CLASS_NEG_NORMAL
        else:
            return FP_CLASS_POS_NORMAL
    ...
    def fms_dp(self, xa, xb, xc):
        ...
        c.reducePrecision(IEEE754_DP_PRECISION)
    ...
    def cmp_sp(self, op, xa, xb):
        ...
        if (op == 'eq'):
            r = (c == 0)
            if (a.nan or b.nan):
                if (a.nan and (a.m == IEEE754_SP_SNAN_MANTISA)): self.cpu.setCSR(CSR_FFLAGS,
                                                                                 CSR_FFLAGS_INVALID_OPERATION_MASK)
                if (b.nan and (b.m == IEEE754_SP_SNAN_MANTISA)): self.cpu.setCSR(CSR_FFLAGS,
                                                                                 CSR_FFLAGS_INVALID_OPERATION_MASK)
                return 0
    ...
    def convert_hp_to_i32_32b(self, v):
        self.cpu.writeCSR(CSR_FFLAGS, 0)
        a = FPNum(v, 'hp')
    ...
    def convert_hp_to_u32_32b(self, v):
        self.cpu.writeCSR(CSR_FFLAGS, 0)
        a = FPNum(v, 'hp')
    ...
    def convert_sp_to_i32_32b(self, v):
        fp = FloatingPointHelper()
        MIN_I32 = -(1 << 31)
        MAX_I32 = (1 << 31) - 1
    ...
    def convert_sp_to_u32_32b(self, v):
        fp = FloatingPointHelper()
        MIN_U32 = 0
        MAX_U32 = (1 << 32) - 1
